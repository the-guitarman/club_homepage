...
unless Repo.get_by(User, login: "administrator") do
  ...
  date = Timex.DateTime.from({1970, 1, 1})
...
# Competitions
competitions_attributes = [
  %{name: "League 1", matches_need_decition: false},
  %{name: "League 2", matches_need_decition: false},
  %{name: "Super Cup", matches_need_decition: true}
]
for competition_attributes <- competitions_attributes do
  unless Repo.get_by(Competition, competition_attributes) do
    changeset = Competition.changeset(%Competition{}, competition_attributes)
    Repo.insert!(changeset)
  end
end
...
unless Repo.get_by(Season, Map.to_list(season_1)) do
  changeset = Season.changeset(%Season{}, season_1)
  Repo.insert(changeset)
end
season_2 = %{name: "2016-2017"}
unless Repo.get_by(Season, Map.to_list(season_2)) do
...
unless Repo.get_by(Team, slug: "#{club_permalink}-men") do
...
unless Repo.get_by(Team, slug: "#{club_permalink}-men-2") do
...
for text_page_key <- text_pages_keys do
  unless Repo.get_by(TextPage, key: text_page_key) do
    changeset = TextPage.changeset(%TextPage{}, %{key: text_page_key})
    Repo.insert(changeset)
  end 
end
