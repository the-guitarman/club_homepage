defmodule ClubHomepage.Email do
  ...
  def secret_text_email(conn, secret) do
    ...
    user_registration_path = ClubHomepageWeb.Router.Helpers.user_path(%Plug.Conn{}, :new, secret: secret.key)
  ...
  def forgot_password_email(conn, user) do
    domain = project_host(conn)
    change_password_path = ClubHomepageWeb.Router.Helpers.change_password_path(%Plug.Conn{}, :change_password, user.id, user.token)
