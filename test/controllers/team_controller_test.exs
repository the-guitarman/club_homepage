defmodule ClubHomepage.TeamControllerTest do
  ...
  @tag login: true
  test "renders form for new resources with login", %{conn: conn, current_user: _current_user} do
    conn = get conn, team_path(conn, :new)
    assert html_response(conn, 200) =~ "Create Team"
  end

  @tag login: true
  test "creates resource and redirects when data is valid with login", %{conn: conn, current_user: _current_user} do
  ...
  @tag login: true
  test "does not create resource and renders errors when data is invalid with login", %{conn: conn, current_user: _current_user} do
    conn = post conn, team_path(conn, :create), team: @invalid_attrs
    assert html_response(conn, 200) =~ "Create Team"
  end

  @tag login: true
  test "shows chosen resource without login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
  ...
  @tag login: true
  test "shows chosen resource with login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
    conn = get conn, team_path(conn, :show, team)
    assert html_response(conn, 200) =~ "Show Team"
  end
  ...
  @tag login: true
  test "renders page not found when id is nonexistent with login", %{conn: conn, current_user: _current_user} do
  ...
  @tag login: true
  test "renders form for editing chosen resource with login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
  ...
  @tag login: true
  test "updates chosen resource and redirects when data is valid with login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
  ...
  @tag login: true
  test "does not update chosen resource and renders errors when data is invalid with login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
  ...
  @tag login: true
  test "try to delete chosen resource with login", %{conn: conn, current_user: _current_user} do
    team = create(:team)
