defmodule ClubHomepage.NewsControllerTest do
  ...
  @tag login: false
  test "shows chosen public resource", %{conn: conn} do
    news = create(:news, public: true)
    conn = get conn, news_path(conn, :show_public, news)
    assert html_response(conn, 200) =~ "<h2>#{news.subject}</h2>"
  end
  ...
  @tag login: false
  test "renders page not found when id is nonexistent for public news", %{conn: conn} do
    assert_error_sent 404, fn ->
      get conn, news_path(conn, :show_public, -1)
    end
  end
